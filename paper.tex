...
\IEEEauthorblockA{Dept.\ of Software Research, xixum.org\\
Grafing bei M\"unchen, Germany\\
...
on known objects it remains at least as specific ($70\%$ vs.\ $33\%$ useful) and
...
space. Instead of a flat list, categories form a \emph{taxonomy}
...
abstracts to \textsf{Vehicle}; only the truly cross-categorical cases (the horse
trailer) become \textsc{unknown}. This graded behavior is the intended one:
maximal specificity subject to a safety floor.
...
segmentation} as a parallel taxonomy and cross-validation layer, confirming or
contradicting each box-level decision against its pixel context (e.g.\ rejecting
a ``vehicle'' whose region segments as sky). Together these move the system
toward the multi-level semantic validation argued for in~\cite{schaller2025}.
