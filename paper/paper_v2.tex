...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{segmentation_qualitative.png}
...
segmentation. The novel deer with no taxonomy leaf abstracts to
\textsf{Living~Being} on the box path and is independently confirmed as a living
being by the segmenter; the loaded trailer is confirmed \textsf{Vehicle}.}
\label{fig:segqual}
\end{figure}
...
\caption{Segmentation cross-check verdict over all box detections (left) and by
box-path outcome (right): confirm / neutral / flag and \emph{zero} conflicts,
