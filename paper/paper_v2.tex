...
Extended version; v1 archived at Zenodo: \href{https://doi.org/10.5281/zenodo.21593472}{doi:10.5281/zenodo.21593472}}}
...
model family (open-vocabulary CLIPSeg~\cite{clipseg2022} rather than YOLO+CLIP),
its agreement with the box path is genuine corroborating evidence, not a
correlated echo of the same features.
...
\begin{itemize}
\item \textsc{confirm}: $f_b$ is high, so the segmentation independently supports
the reported branch.
...
object is not sky-plausible. This is the data-driven form of the ``car above the
...
% Segmentation figures declared early (right after the last results figure) so
% these single-column floats fill the results/discussion pages and the long
% Conclusion text ends cleanly, rather than the figures leaving an end-of-paper
% hole. They are referenced later in Sec.~\ref{sec:results-seg}.
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{segmentation_qual_1.png}
\caption{A car towing a trailer: box path (top) over segmentation (bottom). The
box path reports \textsf{Vehicle} and the segmenter independently paints the same
region as vehicle.}
\label{fig:segqual1}
\end{figure}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{segmentation_qual_2.png}
\caption{Two independent perception paths, per scene: (top) the YOLO+CLIP boxes,
outlined in the segmentation verdict colour with a per-box mark ($\checkmark$
confirm, $\sim$ neutral, $\triangle$ flag), and (bottom) the dense CLIPSeg
segmentation. The novel deer, which has no taxonomy leaf, is abstracted to
\textsf{Living~Being} on the box path and independently confirmed as a living
being ($100\%$ of its object pixels) by the segmenter; the car with trailer
(Fig.~\ref{fig:segqual1}) is confirmed \textsf{Vehicle}.}
\label{fig:segqual}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{segmentation_agreement.png}
\caption{Segmentation cross-check verdict over all box detections (top) and by
box-path outcome (bottom): confirm / neutral / flag and \emph{zero} conflicts,
with no obstacle dropped; corroboration and flags concentrate on the
...
with segmentation enabled (Figs.~\ref{fig:segqual1},~\ref{fig:segqual},~\ref{fig:segagree}). The
...
\textsc{unknown} obstacles are left untouched and never vetoed. Qualitatively, a
novel deer with no taxonomy leaf (Fig.~\ref{fig:segqual}) is abstracted to
\textsf{Living~Being} by the box path and independently confirmed as a living
being ($100\%$ of its object pixels) by the segmenter, while a car with trailer
(Fig.~\ref{fig:segqual1}) is confirmed \textsf{Vehicle}. The flags are the
...
\section{From Cross-Validation to Cross-Supervision}
\label{sec:xsup}
The two paths need not remain mere runtime validators of each other; their
agreement is itself a source of \emph{supervision}. Each \textsc{confirm}
(Fig.~\ref{fig:segagree}) is a high-confidence, automatically generated
pseudo-label that ties a region to a safety-floor category, and each
\textsc{flag} is a hard, self-identified example: precisely the novel or
elevated objects on which 2D appearance fails. Crucially, the labels the box path
hands the segmenter are \emph{safe by construction}: the hierarchy never commits
a wrong leaf, only a correct coarser class, so a giraffe teaches the segmenter
``\textsf{Living~Being} here,'' never a spurious species. The box path thus
becomes an oracle that can \emph{teach} the pixel path on exactly the long-tail
objects neither model was trained for. Cross-validation becomes
\emph{cross-supervision}. In our run the seven confirmations and seven flags
together cover \emph{every} abstracted novel object ($74\%$ of all detections):
confirmations as free positive labels, flags as cases routed to review.

This inverts the usual data economy. Harvesting agreement labels over a large
\emph{unlabeled} driving corpus yields a taxonomy-aligned, safety-graded
segmentation set at no annotation cost, on which the segmentation layer (and, per
the detector-retraining direction below, the box model itself) can be fine-tuned
to \emph{hierarchical} targets. We intend to release the harvested benchmark and
the resulting open-set models openly on the HuggingFace Hub, so the community can
build on a shared, safety-oriented resource instead of re-annotating the long
tail from scratch. Combined with the 3D signal of the next section, this closes a
self-improving loop: geometry and hierarchy supervise appearance, and the
improved appearance models feed back into both perception paths.
...
% Flush any figure still queued onto a page BEFORE the bibliography, so the
% natural order (figures, then references) is preserved -- never figures-after-refs.
\clearpage
